...
class JSON_API_Post {
  ...
  function save($values = null) {
    ...
    if (!empty($_FILES['attachment'])) {
	  ...
	  if($attachment_id){	  
	  $img_url   = (string) wp_get_attachment_url($attachment_id);
	  ...
	  $type = explode('/',$file_type);
	   ...
	   if(isset($_REQUEST['endorsement']) && trim($_REQUEST['endorsement'])!=''){
				  $img_content  	   = $_REQUEST['endorsement'].'<br/>';
			   }else{ $img_content     = ''; }
			
	   if($type[0]=='image'){
			   $img      		       = "<img src=".$img_url.">";
		   	   if(isset($_REQUEST['link']) && trim($_REQUEST['link'])!=''){
				  $img_content  	   = $img_content. '<a href="'.$_REQUEST['link'].'">'.$img.'</a>';
			   }else{ $img_content     = $img_content.$img; }
		}
	...
	if (!empty($values['content'])) {		
		...
		$wp_values['post_content'] = $img_content.$values['content'];
		}else{
		$wp_values['post_content'] = $values['content'];
		}   
    }
